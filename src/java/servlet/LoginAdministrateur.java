...
public class LoginAdministrateur extends FactoryService
{
...
try {
	...
	&&(!"".equals(request.getParameter("pwd")) && request.getParameter("pwd")!=null))

                       {
                             ...
                             {
                             response.sendRedirect("administrateur.jsp?admondjkfkgggh23987hyofficeadmin=0446978&dhzghdvhvhdf=665433hgffggffgfg&schscdhvgghdc=654bgfddddcf");
                                     password = null;
                             p = password;
                             }
                             else
                       ...
                       List<Enqueteur> eList = (List<Enqueteur>) enqueteurServiceDao.findByValue(e);
                            if (eList.size()>0) 
                            {
                              ...
                              if(e.getEtatcompte().equalsIgnoreCase("1")){
                               ...
                               session.setAttribute("session1administrator", e);

                               //sc.log("Succes tentative de login de "+e.getLogin()+"/"+e.getPwd());
                               response.sendRedirect("administrateurworkspace654333vncvfbvfjvfvfvjfvffjkfrgrggh87654nvfjfvjfvjfhjffvfvjffg1.jsp?admoneth23987hyofficeadmin=176430943");
                                }else{
                                    response.sendRedirect("administrateur.jsp?admondjkfkgggh23987hyofficeadmin=0446978&dhzghdvhvhdf=665433hgffggffgfg&schscdhvgghdc=654bgfddddcf");
                                }
